...
function setEnvValue(env: NodeJS.ProcessEnv, key: string, value: string, platform: NodeJS.Platform): void {
  if (platform === "win32") {
...
function deleteEnvValue(env: NodeJS.ProcessEnv, key: string, platform: NodeJS.Platform): void {
  if (platform === "win32") {
    const normalizedKey = key.toLowerCase();
    for (const existingKey of Object.keys(env)) {
      if (existingKey.toLowerCase() === normalizedKey) delete env[existingKey];
